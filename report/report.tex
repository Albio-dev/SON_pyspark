...
\end{algorithm}
	...
	These tests were performed on an 8 core $i5-8250U @1.6GHz$, with 16 GB DDR4 memory $@2400MHz$, but both the driver and the executor were capped at 4 GB. Figures \ref{fig:easybenchmark}, \ref{fig:hardbenchmark} represent execution times with the growth of the datasets.
	...
	Furthermore, we have encountered an issue where the global support was reduced so much that the support in the partitions turned out to be less than what would be corresponding to one basket, which caused an explosion in the number of candidates generated by Apriori, not because they were actually frequent in the partition, but because every item (and the combinations in each basket) always appear at least once. In general this issue happens every time the support of a partition is too low ($\leq$ 5\% in a single partition) because the core is still Apriori and it's very important that the minimum support is high enough to prune the data as the algorithm works.\\
	...
	Another possible issue we encountered was that uneven partition sizes can cause major slowdowns for the same reasons as before.\\
